...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <!-- used mainly for PyLaia which is able to produce the word coordinates!! -->
    ...
    <xsl:template name="page:Word">
        ...
        <xsl:variable name="nrOfStringsAfter" select="count(./following-sibling::Word)"/>
        <xsl:variable name="lastWordOfPreviousLine" select="./parent::TextLine/preceding-sibling::TextLine[1]/page:Word[last()]/TextEquiv/Unicode/text()"/>
        <xsl:variable name="firstWordOfNextLine" select="./parent::TextLine/following-sibling::TextLine[1]/page:Word[1]/TextEquiv/Unicode/text()"/>
        <!--        <xsl:message>
                    <xsl:value-of select="$lastWordOfPreviousLine"/>
                </xsl:message>
                <xsl:message>
                    <xsl:value-of select="$firstWordOfNextLine"/>
                </xsl:message>-->
        <xsl:variable name="wordText" select="./TextEquiv/Unicode/text()"/>
        <xsl:variable name="hyp1">
            <xsl:if test="not($nrOfStringsAfter>0) and string-length($wordText)>1 and $firstWordOfNextLine and (ends-with($wordText, '&#x00AC;') or ends-with($wordText, '&#x2010;') or ends-with($wordText, '&#x002D;') or ends-with($wordText, '&#x2212;') or ends-with($wordText, '&#x201E;') or ends-with($wordText, '&#x003D;'))">
                <!--                <xsl:message>
                                    <xsl:value-of select="string-length($wordText)"/>
                                </xsl:message>-->
                <xsl:value-of select="substring($wordText,1,string-length($wordText)-1)"/>
            </xsl:if>
        </xsl:variable>
        <xsl:variable name="hyp2">
            <xsl:if test="$lastWordOfPreviousLine and string-length($lastWordOfPreviousLine)>1 and not($nrOfStringsBefore>0) and (ends-with($lastWordOfPreviousLine, '&#x00AC;') or ends-with($lastWordOfPreviousLine, '&#x2010;') or ends-with($lastWordOfPreviousLine, '&#x002D;') or ends-with($lastWordOfPreviousLine, '&#x2212;') or ends-with($lastWordOfPreviousLine, '&#x201E;') or ends-with($lastWordOfPreviousLine, '&#x003D;'))">
                <xsl:value-of select="substring($lastWordOfPreviousLine,1,string-length($lastWordOfPreviousLine)-1)"/>
            </xsl:if>
        </xsl:variable>
        <xsl:variable name="subsContent">
            <xsl:choose>
                <xsl:when test="$hyp1 and string-length($hyp1)>0">
                    <xsl:value-of select="concat($hyp1, $firstWordOfNextLine)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:if test="$hyp2 and string-length($hyp2)>0">
                        <xsl:value-of select="concat($hyp2, $wordText)"/>
                    </xsl:if>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        ...
        <String ID="{@id}">
            ...
            <xsl:choose>
                <xsl:when test="$hyp1 and string-length($hyp1)>0">
                    ...
                    <xsl:attribute name="SUBS_CONTENT" select="$subsContent"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:if test="$hyp2 and string-length($hyp2)>0">
                ...
                <String>
                    ...
                    <xsl:if test="not($lastWordOfPreviousLineIfAny = 'NA') and not($lastWordOfPreviousLineIfAny = '')">
                        ...
                        <xsl:attribute name="SUBS_CONTENT"
                                       select="concat(substring($lastWordOfPreviousLineIfAny,1,string-length($lastWordOfPreviousLineIfAny)-1),$actString)"/>
                    </xsl:if>
                    <xsl:if test="$includeTags">
                        <xsl:call-template name="createNamedEntityRef">
                ...
                <String>
                    ...
                    <xsl:choose>
                        <xsl:when
                            ...
                            <xsl:attribute name="SUBS_CONTENT"
                                           select="concat(substring($remainingContent,1,string-length($remainingContent)-1),$firstWordOfNextLineIfAny)"
                            />
                        </xsl:when>
                    </xsl:choose>
                    <xsl:if test="$includeTags">
