...
import static eu.europeana.fulltext.api.caching.CachingUtils.generateETag;
import static eu.europeana.fulltext.api.caching.CachingUtils.generateSimpleETag;
...
public class FTRetrievalController {
    ...
    private ResponseEntity<String> getAnnoPageInfo(String datasetId, String localId, HttpServletRequest request, boolean isJson)
        ...
        // annopage info generates only v3 response
        String eTag = generateETag(modified, REQUEST_VERSION_3 + fts.getSettings().getAppVersion(), true);

        if (CachingUtils.cachingHeadersPresent(request)) {
            ResponseEntity<String> cached = CachingUtils.checkCached(request, modified, eTag);
            if (cached != null) {
    ...
    private ResponseEntity<String> annoPage(
        ...
        ZonedDateTime modified = CachingUtils.dateToZonedUTC(annoPage.getModified());
        String eTag = CachingUtils.generateETag(modified, requestVersion + fts.getSettings().getAppVersion(),
            true);
        ResponseEntity<String> cached = CachingUtils.checkCached(request, modified, eTag);
        if (null != cached) {
            return cached;
    ...
    private ResponseEntity<String> annotation(
        ...
        ZonedDateTime modified = CachingUtils.dateToZonedUTC(annoPage.getModified());
        String eTag = generateETag(modified, requestVersion + fts.getSettings().getAppVersion(),
            true);
        ResponseEntity<String> cached = CachingUtils.checkCached(request, modified, eTag);
        if (cached != null) {
            return cached;
        ...
        headers = CachingUtils.generateHeaders(request, eTag, CachingUtils.zonedDateTimeToString(modified));
        AcceptUtils.addContentTypeToResponseHeader(headers, requestVersion, isJson);
    ...
    private ResponseEntity<String> resource(
        ...
        ZonedDateTime modified = CachingUtils.januarificator();
